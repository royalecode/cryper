...
class MainButton extends StatefulWidget {
  ...
  MainButton({Key? key, required this.text, required this.name, required this.email,
    required this.password, required this.newUser}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Container(
        child: SizedBox(
          ...
          onPressed: () async {
              if (widget.newUser){
                auth.createUserWithEmailAndPassword(email: widget.email,
            ...
            DocumentReference ref =
                ...
                auth.signInWithEmailAndPassword(email: widget.email,
                    password: widget.password);
                Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context) => HomeScreen()));
              }
          },
